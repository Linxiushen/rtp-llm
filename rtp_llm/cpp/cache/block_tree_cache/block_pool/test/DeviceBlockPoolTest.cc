...
static_assert(
    std::is_same_v<decltype(DeviceBlockPoolConfigHelper::createConfigForGroup(
                       std::declval<const CacheConfig&>(),
                       std::declval<const GroupBase&>())),
...
std::shared_ptr<DeviceBlockPoolConfig> makeConfig() {
    ...
    rtp_llm::CacheConfig cache_config    = rtp_llm::test::makeSimpleMhaCacheConfig(kLayerNum,
                                                                                ...
                                                                                /*local_head_num_kv=*/1,
                                                                                /*size_per_head=*/64);
    cache_config.finalizeBlockNums(cache_config.block_num, rtp_llm::RuntimeConfig{});
...
std::shared_ptr<DeviceBlockPoolConfig> makeMixedScaleConfig() {
    ...
    scaled_cfg.finalizeBlockNums(scaled_cfg.block_num, rtp_llm::RuntimeConfig{});
    plain_cfg.finalizeBlockNums(plain_cfg.block_num, rtp_llm::RuntimeConfig{});

    DeviceBlockPoolConfig scaled_pool = DeviceBlockPoolConfigHelper::createConfigForGroup(
        scaled_cfg, scaled_cfg.topology().groups().front());
    DeviceBlockPoolConfig plain_pool = DeviceBlockPoolConfigHelper::createConfigForGroup(
        plain_cfg, plain_cfg.topology().groups().front());
...
CacheConfig makeMtpCacheConfig() {
    ...
    main.finalizeBlockNums(main.block_num, rtp_llm::RuntimeConfig{});
    return main;
}
...
TEST(DeviceBlockPoolTest, MultiLayoutMtpConfigUsesMainBlockCountAndGlobalLayerMapping) {
    ...
    ASSERT_EQ(cache_config.mtp_sub_configs[0]->block_num, 3u);
    ASSERT_EQ(cache_config.mtp_sub_configs[1]->block_num, 3u);

    auto config = std::make_shared<DeviceBlockPoolConfig>(DeviceBlockPoolConfigHelper::createConfigForGroup(
        cache_config, cache_config.topology().groups().front()));
    config->use_device_malloc_backing = true;
