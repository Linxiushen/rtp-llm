...
GenerateStreamPtr MtpExecutor::createMinFakeDecodeStream(int                    max_new_tokens,
    ...
    auto next_seq_len_gpu   = torch::full({1}, 2, int32_gpu);
...
MtpExecutor::MtpExecutor(const EngineInitParams&                        params,
    ...
    LogitsProcessorFactory::init(params.model_config_, params.sp_config.tree_decode_config, params.grammar_config);
    cudaProfilerBegin();
...
void MtpExecutor::populateTargetVerifyHostMetadata(GptModelInputs&      target,
    ...
    // Only legacy callers carry a host mirror. Device-state MTP keeps dynamic
    // lengths on CUDA and supplies only the static query widths above on host.
